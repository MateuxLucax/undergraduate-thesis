...
Conforme dados do Censo 2010 \cite{censo2010}, aproximadamente 46 milhões de brasileiros declararam ter algum grau de dificuldade em pelo menos uma das seguintes habilidades: enxergar, ouvir, caminhar ou subir degraus. Dessas, a habilidade de enxergar e ouvir são primordiais para a inclusão digital dos usuários. Conforme a lei nacional n° 12.965 \cite{lei12965} é obrigatório a adesão de padrões tecnológicos que permitam a comunicação, acessibilidade e interoperabilidade entre sistemas.

Para tecnologias na web, a \cite{w3c} estabeleceu diretrizes de acessibilidade para conteúdo web, conhecidas como WCAG (Web Content Accessibility Guidelines). Essas diretrizes são amplamente adotadas e servem como referência para garantir que os conteúdos web sejam acessíveis a todos os usuários, incluindo pessoas com deficiência (PCDs).

Em um estudo realizado por \cite{accessibilitynativeapps} o método mais utilizado para validação de acessibilidade é manual utilizando ferramentas como leitores de tela, teclados virtuais e outras tecnologias assistivas. Apenas 18,2\% dos testes sendo realizados de forma automática. Segundo dados levantados pela \cite{iaap}, 26,8\% das organizações buscam expandir sua acessibilidade para incluir também aplicações móveis, sendo esse um dos 5 principais objetivos de acessibilidade para 2022.

Ferramentas como linters e analisadores estáticos de código são amplamente utilizadas no desenvolvimento de software para identificar possíveis falhas e melhorias no código assim como descrito por \cite{javascriptlinters} e geralmente desenvolvedores 
não se opõem a utilização de ferramentas de análise estática de código, pois elas ajudam a identificar problemas de forma rápida e eficiente. No contexto de acessibilidade, essas ferramentas podem ser utilizadas para identificar problemas de acessibilidade no código-fonte e fornecer orientações e recomendações para corrigi-los.
...
\subsubsection{Revisão da Literatura}\label{sec:revisao-literatura}
...
A metodologia adotada neste TCC foi baseada nas fases fundamentais da Engenharia de Software propostas por \cite{engenhariadesoftware}, incluindo análise (Engenharia de Requisitos), projeto, programação, testes e validação assim como disposto na figura \ref{fig:metodologia}. Além disso, a execução do trabalho foi gerenciada utilizando a metodologia ágil Scrum para garantir um desenvolvimento iterativo e incremental do projeto. A seguir, detalhamos cada uma das fases e ferramentas utilizadas na metodologia:

Engenharia de Requisitos: nesta fase, foi realizado um mapeamento sistemático da literatura para identificar os requisitos não funcionais de acessibilidade necessários para aplicações móveis. As principais fontes consultadas incluíram artigos científicos, livros e diretrizes de organizações especializadas em acessibilidade. As informações coletadas serviram como base para a definição dos requisitos que orientaram o desenvolvimento da extensão proposta.
...
\begin{itemize}
	...
	\item Capítulo 3 - Trabalhos Correlatos: realiza uma análise comparativa de aplicações que apresentam um contexto similar ao do presente TCC, explorando as similaridades e as diferenças entre elas para aprimorar o entendimento e guiar o desenvolvimento do projeto.
